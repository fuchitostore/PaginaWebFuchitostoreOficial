...
(function () {
  ...
  navWrapper.innerHTML =
    ...
    '<div id="fs-nav-menu">' +
      '<a href="' + basePath + 'account.html" class="fs-menu-item">🧾 Mi perfil</a>' +
      '<a href="' + basePath + 'account.html#micoleccion" class="fs-menu-item">❤️ Mi Colección</a>' +
      '<a href="' + basePath + 'account.html#recompensas" class="fs-menu-item">🏆 FuchiPoints</a>' +
  ...
  var navWrapperRef = navBtn ? navBtn.parentNode : null;
  ...
  document.addEventListener('click', function (e) {
    if (navWrapperRef && !navWrapperRef.contains(e.target)) {
  ...
  window.FS_AUTH.onAuthStateChanged(function (user) {
    if (user) {
      ...
      navBtn.innerHTML = '👤 ... <span id="fs-nav-badge"></span>';
      // Leer nombre y nivel directamente desde Firestore
      window.FS_DB.collection('usuarios').doc(user.uid).get().then(function (snap) {
        var nombre = 'Cuenta';
        var nivel  = 'aficionado';
        if (snap.exists) {
          var data = snap.data();
          // Priorizar nombre guardado en Firestore, luego displayName de Google
          nombre = (data.nombre && data.nombre.trim())
            ? data.nombre.split(' ')[0]
            : (user.displayName ? user.displayName.split(' ')[0] : 'Cuenta');
        ...
        var n = niveles[nivel] || niveles.aficionado;
        navBtn.innerHTML = '👤 ' + nombre + ' <span id="fs-nav-badge">' + n.emoji + ' ' + n.label + '</span>';
      }).catch(function () {
        // Fallback si Firestore falla
        var nombre = user.displayName ? user.displayName.split(' ')[0] : 'Cuenta';
        navBtn.innerHTML = '👤 ' + nombre + ' <span id="fs-nav-badge"></span>';
      });
    } else {
